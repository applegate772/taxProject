...
const STD_DED = { MFJ:30000, HOH:22500, S:15000, MFS:15000 };
const MED_SUR = { MFJ:250000, HOH:200000, S:200000, MFS:125000 };
const QBI_PH  = { MFJ:{lo:383900,hi:483900},
...
const PERIOD = { Q1:{months:3, elapsed:3},
...
function calculateTaxEstimate({
  ...
  const qProfit          = Math.max(qGross - qExp, 0);          // quarterly profit
  const annProfitGross   = qProfit * (12 / monthsQ);            // before salary adjustment

  /* ---- 2. Annualise W-2 wages ---- */
  const annW2SCorp  = w2SCorpYTD * (12 / monthsEL);
  const annW2Other  = w2OtherYTD * (12 / monthsEL);

  /* ---- 3. Remove S-corp salary from business profit to avoid double-count ---- */
  ...
  const seTaxApplies = w2SCorpYTD === 0;        // no S-corp salary ⇒ pay SE tax
  let seTax = 0, halfSE = 0;
  if(seTaxApplies){
    const seBase   = annProfitNet * 0.9235;     // only the profit portion
    const ssCapLeft = Math.max(WAGE_BASE - (annW2SCorp + annW2Other), 0);
    seTax += Math.min(seBase, ssCapLeft) * 0.124;
    seTax += seBase * 0.029;
    if(seBase > MED_SUR[status]) seTax += (seBase - MED_SUR[status]) * 0.009;
    halfSE = seTax / 2;
  }
  ...
  /* ---- 8. Project withholding from YTD alone ---- */
  const projWith = withYTD * (12 / monthsEL);

  /* ---- 9. Quarterly payment ---- */
  const grossTax = incTax + seTax;
  ...
  return {
    ...
    projectedFederalTax: r2(grossTax),
